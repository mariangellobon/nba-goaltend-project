...
video_path = '/Users/mariaangellobon/Desktop/Syncing Video Data/ball_pin_backboard_hard1_dataset3.mov'
data_path = '/Users/mariaangellobon/Desktop/Syncing Video Data/Mike_Dataset3.csv'


# -- LOAD & Preprocess CSV DATA ---
root_df = pd.read_csv(data_path)
# keep only the time and Z-axis acceleration columns
root_df = root_df[["Data Set 3:Time(s)", "Data Set 3:Z-axis acceleration 1(m/s²)"]].copy()
# --- align knock moment between IMU data and video ---
#TODO UPDATE THESE VALUES
imu_knock_time_s = 0.3706       # Time(s) value in the CSV at the knock moment (row 220)
video_knock_time = 6.961        # seconds into the video where the knock occurs
...
root_df["video_time"] = root_df["Data Set 3:Time(s)"] + offset


max_acceleration = root_df["Data Set 3:Z-axis acceleration 1(m/s²)"].max() #used for setting y axis graph
min_acceleration = root_df["Data Set 3:Z-axis acceleration 1(m/s²)"].min() #used for setting y axis lower bound
...
# Crop bottom-left corner before resizing (preserves quality by working on full-res frame)
crop_left = int(rotated_width * 0.12)    # remove 12% from the left
crop_bottom = int(rotated_height * 0.15) # remove 15% from the bottom


# define codec and create VideoWriter object to save video
fourcc = cv.VideoWriter_fourcc(*'mp4v')
out = cv.VideoWriter('/Users/mariaangellobon/Desktop/Syncing Video Data/ball_pin_backboard_hard1_dataset3_overlay3.mov', fourcc, fps, (scale_width, scale_height))
...
while cap.isOpened():
    ...
    if not graph_df.empty:
        ...
        ax.plot(graph_df["video_time"], graph_df["Data Set 3:Z-axis acceleration 1(m/s²)"], color='blue')
        ax.axhline(y=0, color='black', linewidth=0.8, alpha=0.4)
        ax.set_xlim([11, 13])
        ax.set_ylim([min_acceleration - 5, max_acceleration + 5])
        ax.set_title("Z-axis Acceleration (m/s²)", fontsize=10)
        ax.set_xlabel("Time (s)", fontsize=8)
        ax.set_ylabel("Z-axis Acceleration (m/s²)", fontsize=8)
        tick_step = 1  # seconds


        ax.set_xticks([11, 12, 13])
        ax.set_facecolor((1, 1, 1, 0.2))  # make background transparent
        # ax.axis('off')
